/*
 * BlackEra_Mod.dylib — 《黑色纪元》修改器 (编译与作用域修复版)
 */
...
static NSString *ExtractTextFromObject(id obj) {
    ...
    if ([obj isKindOfClass:[NSDictionary class]]) {
        ...
        for (id key in dict) {
            NSString *txt = ExtractTextFromObject(dict[key]);
...
static BOOL ContainsAnnouncementKeywords(NSString *text) {
    NSArray *keywords = @[@"得到了", @"在观看广告时", @"获得了", @"抽取了", @"神兵符碎片", @"极品", @"神器", @"掉落了"];
...
static void BE_Bcloud_CallFunc(id self, SEL _cmd, id arg1, id arg2, void (^block)(id result, NSError *error)) {
    NSString *funcName = [arg1 isKindOfClass:[NSString class]] ? (NSString *)arg1 : nil;
    BOOL shouldBlock = NO;
    if (funcName && ([funcName rangeOfString:@"SendSystemMessage" options:NSCaseInsensitiveSearch].location != NSNotFound ||
                     [funcName rangeOfString:@"broadcast" options:NSCaseInsensitiveSearch].location != NSNotFound)) {
        shouldBlock = YES;
    }
    ...
    if (g_intercept_announce && shouldBlock) {
        AddLog(@"[🛡️] 已在底层静默拦截广播云函数");
        if (block) block(@{}, nil);
        return;
    }
...
static void BE_SocketIO_SendEvent(id self, SEL _cmd, NSString *event, id data) {
    ...
    if (g_intercept_announce && (
        [event rangeOfString:@"reward" options:NSCaseInsensitiveSearch].location != NSNotFound ||
        [event rangeOfString:@"broadcast" options:NSCaseInsensitiveSearch].location != NSNotFound ||
        ContainsAnnouncementKeywords(dataText))) {
        AddLog(@"[🛡️] 已在底层静默拦截Socket上报广播");
        return;
    }
...
- (void)claimSpiritStonesLoop {
    ...
    Method m = FindMethodContains([targetVC class], @"rewardVideoAdDidRewardEffective") ?: FindMethodContains([targetVC class], @"reward");
    if (!m) {
        AddLog(@"[❌] 未找到广告发奖接口");
        return;
    }
...
- (void)triggerMakeActions {
    ...
    for (unsigned int i = 0; i < count; i++) {
        ...
        if ([name rangeOfString:@"click" options:NSCaseInsensitiveSearch].location != NSNotFound ||
            ...
            [name rangeOfString:@"stronger" options:NSCaseInsensitiveSearch].location != NSNotFound) {
            @try {
                UIButton *fakeBtn = [UIButton buttonWithType:UIButtonTypeSystem];
                ((void (*)(id, SEL, id))objc_msgSend)(targetVC, sel, fakeBtn);
                AddLog([NSString stringWithFormat:@"[⚒️] 已触发动作: %@", name]);
            } @catch (...) {}
        }
    }
    if (methods) free(methods);
    ...
    for (unsigned int i = 0; i < count; i++) {
        ...
        if ([name rangeOfString:@"wash" options:NSCaseInsensitiveSearch].location != NSNotFound ||
            ...
            @try {
                UIButton *fakeBtn = [UIButton buttonWithType:UIButtonTypeSystem];
                ((void (*)(id, SEL, id))objc_msgSend)(targetVC, sel, fakeBtn);
                AddLog([NSString stringWithFormat:@"[🐾] 已触发灵宠动作: %@", name]);
            } @catch (...) {}
        }
    }
    if (methods) free(methods);
...
+ (void)showFloatingBall {
    ...
    dispatch_async(dispatch_get_main_queue(), ^{
        ...
        [containerView addSubview:s_ball];
        ...
        // 控制面板
        [self buildPanel:containerView bounds:bounds];
...
+ (void)clickToggleAnnounce:(UIButton *)btn {
    ...
    [btn setTitle:g_intercept_announce ? @"🔇 上行全服公告源头拦截:【已开启】" : @"🔊 上行全服公告源头拦截:【已关闭】" forState:UIControlStateNormal];
    AddLog(g_intercept_announce ? @"[🛡️] 公告源头拦截已开启" : @"[🛡️] 公告源头拦截已关闭");
}
